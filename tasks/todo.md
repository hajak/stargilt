...
## Round 72 — v0.13.12-exp: `hidden` never hid (2026-08-06)
- [x] "Last run I died. I just reloaded..." + screenshot → the screenshot ITSELF was the diagnosis: CONTINUE with an EMPTY note and PLAY still gold/primary. Both are impossible if initStartMenu()'s save branch had run (it fills the note, demotes PLAY, binds the click). So the button was never unhidden by code — it was NEVER HIDDEN.
- [x] ROOT CAUSE: `#sm-continue` carries the `hidden` attribute, but `.sm-item{display:flex}` is an AUTHOR rule and the UA's `[hidden]{display:none}` loses to it. The button has been on screen for every player since it was written, with or without a save — and was a DEAD button (no click handler is bound without a save). Proved headlessly on a device with save===null: hidden property true, computed display flex, on screen true.
- [x] FIX: `[hidden]{display:none!important}` in the reset — the attribute now means what it says everywhere, incl. #insp-skip and anything added later.
...
## Round 76 — v0.13.16-exp: eight playtest notes (2026-08-07)
- [x] (1) ANALYTICS exit: ✕ + scrim-click (hiscore scrim too). (2) Honor roll → aligned grid columns (rank|★|smith|days|stage|when + header). (3) Cold-open captions text-wrap:balance. (4) Grimoire FIRST + modal freeze; gift beat waits for the book to close. (5) Undo slot reserved (.ghosted visibility) — no HUD jump. (6) Take-back taught in Grimoire ch. II + glossary on the button. (7) Sealed Hearth live line: "−1 ❖ TODAY · N ❖ TO SPEND" (was "1 FEWER CARD" — read as hand size). (8) ONE death screen: ledger IS the death screen (verdict+epitaph+predecessors in masthead; #gameover trial-only) + THE REPORT IS EARNED (curve at day 6, autopsy+work at lock 1, deck+bench at lock 2) + dossier drops mono for Fraunces/Alegreya.
- [x] Suite 149 → 151 across 15 files, green. Visual pass: death ledger masthead + earned seals + honor-roll columns.
